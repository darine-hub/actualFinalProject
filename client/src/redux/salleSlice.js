...
export const getsalles = createAsyncThunk(
  ...
  async (info, { rejectWithValue }) => {
    try {
      const res = await axios.get('http://localhost:5000/reservation/getsalles',{
...
export const getsallesbydept = createAsyncThunk(
  ...
  async (info, { rejectWithValue }) => {
    try {
      const res = await axios.get(`http://localhost:5000/reservation/getsallesbydept/${info}`,{
        headers: { token: localStorage.getItem('token') }});
      return res.data;
    } catch (error) {
      return rejectWithValue(error.response.data.message);
...
export const addSalle = createAsyncThunk(
  ...
  async (info, { rejectWithValue , dispatch }) => {
    try {
      ...
      const res = await axios.post('http://localhost:5000/reservation/addSalle', info ,{
        headers: { token: localStorage.getItem('token') }
      }) 
      console.log(info)
      dispatch(getsalles())
