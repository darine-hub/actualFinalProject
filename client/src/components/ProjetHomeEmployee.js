...
const ProjetHomeEmployee = ({history}) => {
    ...
    let filtredtasks = task.tasks && task.tasks.filter(task=>task.employee._id === user.userInfo._id).map(elm=>elm.project._id)
  let filtredProject =project.projects && project.projects.filter(project=>filtredtasks.includes(project._id)).slice(-3, project.projects.length)
...
export default ProjetHomeEmployee
